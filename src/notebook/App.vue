...
interface RedTeamCase {
  ...
  /**
   * 期望结果：
   * - block: 必须执行失败（错误信息匹配 expectMatch）
   * - pass: 必须执行成功（stdout 匹配 expectMatch）
   * - timeout-then-interrupt: 跑起来后用户按"中断"，须在 5s 内得到 KeyboardInterrupt
   */
...
onMounted(async () => {
  ...
  try {
    ...
    append(
      `[host] crossOriginIsolated=${info.crossOriginIsolated} sabSupported=${info.sabSupported}`,
    )
  } catch (err) {
...
const runCode = async (code: string, label: string): Promise<ExecResult> => {
  ...
  if (result.errorType) {
    append(`[error:${result.errorType}] ${result.errorMessage ?? ''}`)
  }
...
const runCase = async (c: RedTeamCase) => {
  ...
  if (c.expect === 'pass') {
    ...
    // block 用例：要求 stdout 全部命中 BLOCKED（或 ImportError 抛出）
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <span
                    class="rounded-full border px-1.5 py-0.5 text-[10px] font-mono"
                    :class="statusBadge(caseStatus[c.id]).cls"
                  >{{ statusBadge(caseStatus[c.id]).text }}</span>
                </div>
