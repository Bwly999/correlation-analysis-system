...
const buildPlanningSystemPrompt = () =>
  [
    ...
    `在输出最终计划前，必须至少调用 ${buildWorkflowToolAlias('workflow_get_session_context')}、${buildWorkflowToolAlias('workflow_get_node_catalog')}，必要时调用 ${buildWorkflowToolAlias('workflow_list_data_sources')}、${buildWorkflowToolAlias('workflow_get_data_source_schema')}，并用 ${buildWorkflowToolAlias('workflow_validate_plan')} 校验最终计划。`,
    `如需确认节点属性、候选节点或连接约束，可调用 ${buildWorkflowToolAlias('workflow_get_node_definition')}、${buildWorkflowToolAlias('workflow_search_nodes')}、${buildWorkflowToolAlias('workflow_get_node')}、${buildWorkflowToolAlias('workflow_get_node_options')}。`,
