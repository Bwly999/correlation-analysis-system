...
const ensureWorkflow = async (userId: string, workflowId: string) => {
...
const buildExecutionRecordFromResults = (
  ...
  const nodes = workflow.nodes.map((node) => {
    ...
    return {
      ...cloneValue(node),
      output: result?.success ? { kind: result.resultKind ?? 'unknown', payload: result.result ?? result.sampleRows ?? [] } : null,
...
export const workflowMcpRuntime = {
  ...
  async createWorkflow(userId: string, input?: { workflowId?: string; name?: string }) {
    ...
    await saveUserWorkflow(userId, workflow)
    return workflow
  },
  ...
  async updatePartialWorkflow(
    ...
    const existing = await ensureWorkflow(userId, workflowId)
    ...
    await saveUserWorkflow(userId, updatedWorkflow)
  ...
  async updateFullWorkflow(userId: string, workflow: SavedWorkflowLike) {
    ...
    await saveUserWorkflow(userId, nextWorkflow)
    return {
      ok: true,
  ...
  async validateWorkflow(userId: string, input: { workflowId?: string; workflowSnapshot?: WorkflowSnapshot }) {
    const workflow = input.workflowSnapshot
      ...
      : toWorkflowSnapshot(await ensureWorkflow(userId, input.workflowId ?? ''))
    if (!workflow) {
      return {
        valid: false,
  ...
  async debugNode(
    ...
    const workflow = await ensureWorkflow(userId, input.workflowId)
    ...
    const executionResults = await executeNodesForAgent(plan, {
      ...sessionRequest,
  ...
  async testWorkflow(userId: string, sessionRequest: WorkflowAiPlanRequest, input: { workflowId: string }) {
    ...
    const results = await executeNodesForAgent(plan, {
      ...sessionRequest,
      workflowSnapshot,
    }, () => undefined)

    const record = buildExecutionRecordFromResults(workflow, results)
    await saveUserHistory(userId, record)
  ...
  async workflowVersions(
    ...
    if (mode === 'list') {
      return {
        ...
        items: await getUserWorkflowVersions(userId, input.workflowId),
      }
    }
    ...
    if (mode === 'get') {
      return {
        ...
        version: await getUserWorkflowVersion(userId, input.workflowId, input.versionId ?? ''),
      }
    }
    ...
    return {
      workflowId: input.workflowId,
      versionId: input.versionId ?? '',
      result: await rollbackUserWorkflowVersion(userId, input.workflowId, input.versionId ?? ''),
    }
  },
}
