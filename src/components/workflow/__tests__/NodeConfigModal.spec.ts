...
describe("NodeConfigModal", () => {
  ...
  it("shows runtime settings for trigger nodes and allows resetting saved runtime inputs", async () => {
    ...
    expect(wrapper.text()).toContain("允许保存启动参数");
  ...
  it("persists runtime input values when applying trigger node settings by default", async () => {
    const store = useWorkflowStore();
    store.nodes = [
      {
    ...
    expect(store.nodes[0]?.data.config.timeRange).toEqual(selectedRange);
    expect(store.nodes[0]?.data.config.materialType).toBe("成品");
  });

  it("does not persist runtime input values when trigger persistence is disabled", async () => {
    const store = useWorkflowStore();
    store.nodes = [
      {
        id: "neighbor-system-node",
