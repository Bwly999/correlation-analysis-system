...
const statusColors = computed(() => {
  switch (props.data.status) {
    case 'running': return 'border-indigo-500 ring-2 ring-indigo-500/20 shadow-sm'
    case 'success': return 'border-emerald-500 ring-1 ring-emerald-500/20 shadow-sm'
    case 'error': return 'border-rose-500 ring-1 ring-rose-500/20 shadow-sm'
    default: return 'border-slate-200 hover:border-slate-300 hover:shadow-md'
  }
})
...
<template>
  ...
  >
    ...
    <div 
      ...
      @click="openConfig"
    >
      <NodeIcon :type="props.data.type" :size="64" class="bg-transparent text-slate-700 group-hover:scale-105 transition-transform duration-300" />
      
      <!-- Pin 状态图标 -->
      <div v-if="isPinned" class="absolute -top-2 -right-2 bg-amber-100 text-amber-600 rounded-full p-1.5 border border-amber-200 shadow-sm z-20">
    ...
    <!-- 自定义命名标题 -->
    <div class="absolute top-[100%] left-1/2 -translate-x-1/2 mt-3 w-[200px] flex flex-col items-center z-20 text-center font-sans">
      ...
      />
      <div class="text-[10px] text-slate-400 font-medium uppercase tracking-wider truncate w-full mt-0.5">
        {{ props.data.type.replace('-', ' ') }}
      </div>
    </div>

    <!-- 连接点 -->
    <Handle v-if="!isTrigger" type="target" :position="Position.Left" class="n8n-handle !-left-1.5" style="z-index: 50;" />
    <Handle v-if="!isTerminal" type="source" :position="Position.Right" class="n8n-handle !-right-1.5" style="z-index: 50;" />
