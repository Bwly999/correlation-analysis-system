  ...
  <header class="absolute top-0 left-0 right-0 h-[56px] bg-white border-b border-slate-200 z-[100] flex items-center justify-between px-6">
    ...
    <div class="flex items-center gap-4">
      ...
      <div class="flex items-center gap-2 group relative">
        ...
        />
        <Edit2 v-if="!store.isHistoryMode" size="12" class="text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity absolute right-2 pointer-events-none" />
        ...
        <Popover ref="historyPopover" class="w-[320px] shadow-2xl border border-slate-200 rounded-xl overflow-hidden mt-2">
          <div class="p-3 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
            <span class="text-[12px] font-bold text-slate-700 uppercase tracking-wider">运行历史 (最近20条)</span>
            ...
            >
              <div class="flex items-center justify-between mb-1">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-[13px] font-medium text-slate-700">{{ new Date(record.startTime).toLocaleTimeString() }}</span>
                </div>
                <span class="text-[11px] text-slate-400 font-mono">{{ formatDuration(record.duration) }}</span>
              </div>
              <div class="text-[11px] text-slate-500 flex justify-between items-center">
                ...
                <span class="opacity-0 group-hover:opacity-100 text-indigo-600 font-semibold transition-opacity">查看详情 →</span>
              </div>
            </div>
          </div>
        </Popover>
    ...
    <div class="flex items-center gap-3">
      ...
      <div 
        ...
        :class="store.isHistoryMode ? 'bg-amber-50 border-amber-200 shadow-sm' : 'bg-slate-50 border-slate-200'"
      >
        <div 
          class="w-2 h-2 rounded-full"
          :class="store.isHistoryMode ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'"
        ></div>
        <span class="text-[11px] font-bold" :class="store.isHistoryMode ? 'text-amber-700' : 'text-slate-600'">
