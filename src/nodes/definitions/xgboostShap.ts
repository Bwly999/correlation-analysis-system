...
export const xgboostShapNode: NodeDefinition = {
  ...
  properties: [
    ...
    },
    {
      ...
      options: [
        { name: '不处理', value: 'none' },
        { name: 'IQR 区间剔除', value: 'iqr' },
        { name: '孤立森林', value: 'isolation_forest' },
      ],
      description: '选择建模前是否剔除异常样本；默认不处理，避免无感知丢弃数据。',
    },
    {
      name: 'iqrThreshold',
      displayName: 'IQR 倍数',
      ...
      description: 'IQR 异常值边界倍数，后端会限制在 0.5 到 5 之间。',
    },
    {
      name: 'outlierContamination',
      displayName: '预估异常比例',
      type: 'number',
      default: 0.05,
      numberMode: 'decimal',
      displayIf: (config) => config.outlierMethod === 'isolation_forest',
      description: '孤立森林预估异常样本比例，后端会限制在 0.001 到 0.5 之间。',
    },
    {
      name: 'outlierNEstimators',
      displayName: '孤立森林树数量',
      type: 'number',
      default: 100,
      displayIf: (config) => config.outlierMethod === 'isolation_forest',
      description: '孤立森林使用的树数量，后端会限制在 50 到 1000 之间。',
    },
    {
      name: 'outlierMaxSamples',
      displayName: '每棵树采样量',
      type: 'options',
      default: 'auto',
      options: [
      ...
      displayIf: (config) => config.outlierMethod === 'isolation_forest',
      description: '控制孤立森林每棵树使用的样本量，默认由算法自动决定。',
    },
    {
      name: 'nEstimators',
      displayName: '树数量',
      ...
      description: 'XGBoost 基础模型的树数量，数值越大训练越慢，后端只作为白名单参数使用。',
    },
    {
      name: 'learningRate',
      displayName: '学习率',
      ...
      numberMode: 'decimal',
      group: ADVANCED_GROUP,
      description: '控制每棵树对最终模型的贡献，后端会限制在 0.001 到 0.5 之间。',
    },
    {
      name: 'maxDepth',
      displayName: '最大树深',
      ...
      default: 6,
      group: ADVANCED_GROUP,
      description: '控制单棵树的最大深度，后端会限制在 2 到 12 之间。',
    },
    {
      name: 'testSize',
      displayName: '测试集比例',
    ...
    },
    {
      ...
      default: 42,
      group: ADVANCED_GROUP,
      description: '控制训练、调参和 SHAP 采样的随机性，便于复现实验结果。',
    },
    {
      name: 'shapSampleLimit',
      displayName: 'SHAP 采样上限',
      type: 'number',
      default: 2000,
      group: ADVANCED_GROUP,
      description: '大样本计算 SHAP 时的最大采样量，后端最小限制为 100。',
    },
    {
      ...
      default: true,
      group: ADVANCED_GROUP,
      description: '当初始 R² 低于阈值时是否触发后端默认搜索网格调参。',
    },
    {
      name: 'autoTuneThreshold',
      displayName: '自动调参阈值',
      type: 'number',
      default: 0.6,
      numberMode: 'decimal',
      group: ADVANCED_GROUP,
      description: '初始 R² 低于该值时触发自动调参，后端会限制在 0 到 1 之间。',
    },
    {
      ...
      default: 20,
      group: ADVANCED_GROUP,
      description: '自动调参时随机搜索的迭代次数，后端保留默认搜索空间。',
    },
    {
      name: 'tuningCv',
      displayName: '交叉验证折数',
      ...
      default: 5,
      group: ADVANCED_GROUP,
      description: '自动调参时的交叉验证折数，样本较少时建议保持较小值。',
    },
  ],
